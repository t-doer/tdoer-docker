...
* Java Base Image: **tdoer/alpine-jdk8-jre-java:1.0.2**
* Port: **8080**
* Documentation, examples, host-manager, manager and ROOT have been removed
* Command: `/opt/tomcat/bin/deploy-and-run.sh` which links .war files from `/deployments`, say to
  `/opt/tomcat/webapps` and then calls `/opt/tomcat/bin/catalina.sh run`
* Command: `/opt/tomcat/bin/deploy-and-run.sh` which links .war files from `/deployments` to
